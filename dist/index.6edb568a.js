(function($) {
    $.fn.fancyknob = function(props) {
        ...
        return this.each(function() {
            ...
            var angle = 0, minangle = 0, maxangle = 280, rangeelem = $(this), minval = parseFloat($(this).prop('min')), maxval = parseFloat($(this).prop('max')), knob = $(this).next().find('.knob'), doc = $(document), step = parseFloat($(this).prop('step')) || 1, a1, b1, deg1, tmp1, rad2deg1 = 180 / Math.PI, startDeg = -1, currentDeg = 0, rotation = 0, lastDeg = 0, decimal_digits = 0, x_str = step.toString().split('.')[1];
            if (x_str !== undefined) decimal_digits = x_str.length;
            ...
            $(this).on("input change", function() {
                ...
                } else angle = Math.round($(this).val() / maxval * 280);
            ...
            $(this).next().bind('DOMMouseScroll', function(e) {
                if (e.originalEvent.detail > 0) moveKnob('down', $(this), false);
                else moveKnob('up', $(this), false);
                return false;
            });
            $(this).next().bind('mousewheel', function(e) {
                if (e.originalEvent.wheelDelta < 0) moveKnob('down', $(this));
                else moveKnob('up', $(this));
                return false;
            });
            knob.on('mousedown', function(e1) {
                e1.preventDefault();
            ...
            knob.on('touchstart', function(e2) {
                e2.preventDefault();
                // not appropriate to use $(this).offset as the offset change when $(this) rotated
                var offset = $(this).parent(".knob-surround").offset();
                var center = {
                ...
                doc.on('touchmove.rem ', function(e) {
                    a1 = center.y - e.originalEvent.touches[0].pageY;
                    b1 = center.x - e.originalEvent.touches[0].pageX;
                    deg1 = Math.atan2(a1, b1) * rad2deg1;
                    if (deg1 < 0) deg1 = 360 + deg1;
                    if (startDeg == -1) startDeg = deg1;
                    tmp1 = Math.floor(deg1 - startDeg + rotation);
                    if (tmp1 < 0) tmp1 = 360 + tmp1;
                    else if (tmp1 > 280) tmp1 = tmp1 % 360;
                    if (Math.abs(tmp1 - lastDeg) > 180) return false;
                    angle = tmp1;
                    if (angle > maxangle) angle = maxangle;
                    setAngle(knob.parent(), true);
                    currentDeg = tmp1;
            ...
            function setAngle(elem, mode) {
                ...
                if (minval < 0) {
                    ...
                    if (angle === 0) elem.find('.tick').removeClass('activetick');
                }
                var pc;
                if (mode) {
                    if (minval < 0) {
                        ...
                        } else if (angle > 140) pc = angle / 140 * maxval - parseInt(maxval);
                        else pc = 0;
                    } else pc = angle / 280 * maxval;
...
//# sourceMappingURL=index.6edb568a.js.map
